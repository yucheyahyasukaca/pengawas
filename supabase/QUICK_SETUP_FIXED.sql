-- ============================================
-- QUICK SETUP SQL - IDEMPOTENT VERSION (Updated with Approval)
-- ============================================
...
-- 3. Tambah kolom status_approval jika belum ada (untuk table yang sudah ada)
DO $$ BEGIN
...
-- 7. Function untuk update updated_at
...
-- 8. Trigger untuk auto-update updated_at
...
-- 9. Function untuk sync user dari auth.users
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.users (id, email, role, status_approval)
    VALUES (
      NEW.id,
      COALESCE(NEW.email, ''),
      'sekolah'::user_role,
      'approved'  -- Default approved untuk non-pengawas
    )
...
-- 10. Trigger untuk auto-create user
...
-- 12. Drop existing policies if exist
...
-- 13. Buat policy khusus untuk INSERT via trigger (PENTING!)
CREATE POLICY "Allow trigger to insert users"
  ON public.users
  FOR INSERT
  WITH CHECK (true);

-- 14. Buat policies untuk SELECT dan UPDATE
...
-- 16. Function untuk approve/reject pengawas
