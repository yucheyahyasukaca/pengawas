...
import { School, CheckCircle2, XCircle, Clock, Mail, Search, Filter, MapPin } from "lucide-react";
...
export default function ApprovalSekolahPage() {
  ...
  const loadSekolahPending = async () => {
    try {
      ...
      const response = await fetch('/api/pengawas/sekolah-pending');
  ...
  const handleApproval = async (sekolahUserId: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      const url = `/api/pengawas/approve-sekolah/${encodeURIComponent(sekolahUserId)}`;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <div className="inline-flex size-12 animate-spin items-center justify-center rounded-full border-4 border-blue-200 border-t-blue-600">
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Approval Pendaftaran Sekolah Binaan</h1>
          <p className="text-sm text-slate-600 mt-1">
            Review dan setujui pendaftaran sekolah binaan Anda
          </p>
        </div>
        {pendingCount > 0 && (
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm transition-all focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          />
        </div>
      ...
      {!error && sekolahPending.length === 0 ? (
        <Card className="border border-blue-200 bg-white shadow-md shadow-blue-100/70">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Semua sekolah binaan sudah diproses atau belum ada pendaftaran baru
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <>
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody className="divide-y divide-blue-100">
                      {filteredSekolah.map((sekolah) => (
                        <tr key={sekolah.id} className="hover:bg-blue-50/50 transition-colors">
          ...
          <div className="grid gap-4 md:hidden">
            {filteredSekolah.map((sekolah) => (
              <Card
                key={sekolah.id}
                className="border border-blue-200 bg-white shadow-md shadow-blue-100/70"
              >
